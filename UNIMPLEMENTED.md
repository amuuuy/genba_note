...
| C-3 | RevenueCat Public Key 未設定 | 一部完了（Offering設定済み、Entitlement要確認、Public Key名修正待ち） | M-04-04/13 [~] → M-02-04/07 |
...
| M-5 | 楽天 API RAKUTEN_APP_ID 確認 | 登録済み ✅ | M-02-05/07 [x] |
...
| M-9 | Gemini API GEMINI_API_KEY 確認 | 登録済み ✅ | M-02-06/07 [x] |
| M-10 | Edge Function レートリミッタ IP偽装耐性 | 実装完了 | 除外（auth.uid ベースに移行済み） |
| M-11 | CORS ワイルドカード | 実装完了 | 除外（Supabase デフォルト CORS 使用） |
| M-12 | Sentry（クラッシュレポート）テストクラッシュ確認 | SDK完了 / DSN登録済み / テスト未実施 | SDK: 除外、DSN: M-02-07/07 [~] |
...
| MS4 | 外部サービス設定 | 一部完了（M-02-01[x], M-02-05[x], M-02-06[x], M-04-01[x], M-04-02[x], M-02-03[~], M-02-07[~], M-04-04[~]） | M-02/07, M-04-01/13〜M-04-04/13 |
| MS5 | ストア準備 | 一部完了（M-04-12/13, M-04-13/13完了） | M-03/07, M-04-05/13〜M-04-11/13 |
| MS6 | CI/CD + ブランチ保護 | 完了 ✅ | workflows: 除外（完了）、保護: M-05-01/01 [x] |
...
| M25 2-2 | RevenueCat 本番設定 | ASC商品作成済み / Offering設定済み / Entitlement要確認 | M-04-02/13 [x], M-04-04/13 [~], M-02-04/07 |
...
| 4.3 | サブスクリプション価格決定 | 完了 ✅ | M-04-01/13 [x]（月額¥480/年額¥4,800） |
| 4.3 | ASC サブスク商品作成 | 完了 ✅ | M-04-02/13 [x] |
| 4.3 | Google Play サブスク商品作成 | iOS先行のため後回し | M-04-03/13 |
| 4.3 | RevenueCat 商品紐付け | Offering設定済み / Entitlement要確認 | M-04-04/13 [~] |
...
### M-01-03/03: expo-updates 導入 ⚠️ OBSOLETE（2026-04-26 M1/C13 で廃止）

> **方針変更**: M1 v1.0.1 リリース時に OTA を無効化（Option A 採択）。詳細は `M1_V1_0_1_RELEASE_FIXES.md` の B7 参照。
> `expo-updates` 依存は Expo SDK 同梱で残置するが、`app.json` の `updates` ブロックは削除済み。`runtimeVersion` のみ EAS Build 用に残置。
> 親友のみ運用で 24-48h の App Store 審査ロスを許容できるため、プライバシー申告を「Sentry のみ」でシンプルに保つことを優先。

- **対象**: `genba-note/package.json`, `genba-note/app.json`
- **現状（v1.0.1 以降）**: `expo-updates` 依存は残置、`app.json` の `updates` ブロックは削除（OTA 無効化）
- **完了条件（旧）**: `jq '.dependencies["expo-updates"]' genba-note/package.json` → バージョン文字列 + `jq '.expo.updates' genba-note/app.json` → 非null
- **新ステータス**: 本タスクは廃止。OTA を再導入する場合は新規タスクとして起票し、プライバシー申告も同時に更新すること
...
| M-02-04/07 | RevenueCat EAS Secrets 登録 | C-3 | M | [ ] ⚠️ 現在 `REVENUECAT_PUBLIC_API_KEY` で登録済みだがコードは `EXPO_PUBLIC_REVENUECAT_PUBLIC_KEY` を期待。名前修正が必要 |
| M-02-05/07 | Rakuten API RAKUTEN_APP_ID 確認・登録 | M-5 | S | [x] Supabase Secrets 登録済み |
| M-02-06/07 | Gemini API GEMINI_API_KEY 確認・登録 | M-9 | S | [x] Supabase Secrets 登録済み |
| M-02-07/07 | Sentry（クラッシュレポート）テストクラッシュ確認（DSN EAS環境変数設定） | M-12 | S | [~] DSN登録済み、テストクラッシュ確認は実機ビルド時 |
...
- ⚠️ **要修正**: 現在 `REVENUECAT_PUBLIC_API_KEY` の名前で EAS Secrets に登録済み。コード（`_layout.tsx:62`）は `EXPO_PUBLIC_REVENUECAT_PUBLIC_KEY` を期待するため、M-04-04/13 完了後に以下を実行：
  ```bash
  eas secret:delete --name REVENUECAT_PUBLIC_API_KEY
  eas secret:create --name EXPO_PUBLIC_REVENUECAT_PUBLIC_KEY --value <key>
  ```

### M-02-05/07: Rakuten API RAKUTEN_APP_ID 確認 [x]

- `supabase secrets list` で `RAKUTEN_APP_ID` の存在を確認 ✅（2026-03-05 登録済み）
- **完了条件**: `supabase secrets list` に `RAKUTEN_APP_ID` が表示 ✅（本番疎通確認は M-06/07 最終検証で実施）

### M-02-06/07: Gemini API GEMINI_API_KEY 確認 [x]

- `supabase secrets list` で `GEMINI_API_KEY` の存在を確認 ✅（2026-03-05 登録済み）
- **完了条件**: `supabase secrets list` に `GEMINI_API_KEY` が表示 ✅（本番疎通確認は M-06/07 最終検証で実施）

### M-02-07/07: Sentry（クラッシュレポート）テストクラッシュ確認（DSN EAS環境変数設定） [~]

- EAS Build の環境変数に `EXPO_PUBLIC_SENTRY_DSN` を設定 ✅（2026-03-15 登録済み）
- テストクラッシュでダッシュボード受信確認 → 実機ビルド時に実施
- **完了条件**: `eas secret:list` に `EXPO_PUBLIC_SENTRY_DSN` が表示 ✅ + Sentry ダッシュボードにテストイベント受信（M-06/07 で確認）
...
| Task | 内容 | 旧Issue | 規模 | 状態 |
|------|------|---------|------|------|
| M-04-01/13 | サブスクリプション価格決定 | MS4 | S | [x] 月額¥480 / 年額¥4,800 |
| M-04-02/13 | App Store Connect サブスク商品作成 | MS4 | M | [x] `genbanote_pro_monthly_v1` + 年額商品 作成済み（2026-03-10） |
| M-04-03/13 | Google Play Console サブスク商品作成 | MS4 | M | [ ] iOS先行のため後回し |
| M-04-04/13 | RevenueCat プロジェクト作成 + 商品紐付け（Entitlement `pro` + Offering） | MS4 | M | [~] Offering default に Monthly/Yearly パッケージ設定済み（2026-03-10）。Entitlement `pro` 要確認 |
...
### M-04-01/13: サブスクリプション価格決定 [x]

- **確定価格**: 月額¥480 / 年額¥4,800（年額は月額比 約17%割引）
- **完了条件**: 月額・年額の価格が確定し、文書化されている ✅
...
| Task | 内容 | 旧Issue | 規模 | 状態 |
|------|------|---------|------|------|
| M-05-01/01 | main ブランチ保護設定 | MS6 | S | [x] PRレビュー必須(1名) + test statusチェック必須 + enforce_admins有効 |
...
- [~] M-01-03/03: ⚠️ OBSOLETE（M1/C13 で OTA 無効化、v1.0.1 以降は app.json の updates 設定なし）
- [ ] `cd genba-note && npx jest` → 0 failures
- [ ] `cd genba-note && npx tsc --noEmit` → exit code 0
...
- [x] M-02-05/07: `supabase secrets list` に `RAKUTEN_APP_ID` が存在
- [x] M-02-06/07: `supabase secrets list` に `GEMINI_API_KEY` が存在
- [~] M-02-07/07: Sentry（クラッシュレポート）テストクラッシュ確認 — `eas secret:list` に `EXPO_PUBLIC_SENTRY_DSN` が存在 ✅ / テストクラッシュイベント受信確認は実機ビルド時
...
**M-04/07 ストア申請準備:**
- [x] M-04-01/13: サブスクリプション価格が確定・文書化済み（月額¥480 / 年額¥4,800）
- [x] M-04-02/13: App Store Connect でサブスク商品が作成済み（`genbanote_pro_monthly_v1` + 年額）
- [ ] M-04-03/13: Google Play Console でサブスク商品が作成済み（iOS先行のため後回し）
- [~] M-04-04/13: RevenueCat で Offering（Monthly/Yearly）が設定済み。Entitlement `pro` 要確認
...
- [x] M-05-01/01: main ブランチ保護が有効（PRレビュー1名必須 + test statusチェック必須 + enforce_admins有効）
...
| M-02/07 外部サービス設定 | 7 | `[Human]` | 3/7完了+2部分完了（M-02-01[x], M-02-05[x], M-02-06[x], M-02-03[~], M-02-07[~], M-02-02[ ], M-02-04[ ]） |
| M-03/07 ドメイン&法務 | 4 | `[Human/Both]` | 0/4完了+3件部分/要確認（ドメイン[?], ホスティング[?], HTML作成済み[~]） |
| M-04/07 ストア申請準備 | 13 | `[Human/Both]` | 4/13完了+1部分完了（M-04-01[x], M-04-02[x], M-04-04[~], M-04-12[x], M-04-13[x]） |
| M-05/07 ブランチ保護 | 1 | `[Human]` | 完了 |
