...
> M24-25 / MS1〜MS3 / MS6(workflows) は実装完了済み — 本文書には未完了項目のみ記載
...
| C-2 | EAS Submit 設定が空 | 未実施 | M-02-03/07 |
...
| M-6 | 資材検索（楽天）Free回数制限 | server完了 / client未実装 | server: 除外、client: M-01-01/03 |
...
| M-13 | ストアメタデータ未作成 | ドラフト完了 | ドラフト: 除外、スクショ: M-04-07/13、レビュー: M-04-12/13 |
...
| MS2 | セキュリティ（H-4, M-7 config, H-1, M-10, M-6） | 5/6完了 | M-6 client → M-01-01/03 |
| MS3 | ビジネスロジック（M-8, M-11, M-3） | 全3項目完了 | 除外（完了済み） |
| MS4 | 外部サービス設定 | 未実施 | M-02/07, M-04-01/13〜M-04-04/13 |
| MS5 | ストア準備 | 未実施 | M-03/07, M-04-05/13〜M-04-13/13 |
...
| M25 2-3 | App Store メタデータ | 未実施 | M-04-07/13〜M-04-13/13 |
...
| セクション | 内容 | 状態 | 対応タスク / 除外理由 |
|-----------|------|------|----------------------|
| 4.1 | Apple Developer Program 登録 | 未実施 | M-02-01/07 |
| 4.1 | Google Play Developer 登録 | 未実施 | M-02-02/07 |
| 4.2 | ドメイン取得 | 未実施 | M-03-01/04 |
| 4.2 | 静的ホスティング設定 | 未実施 | M-03-02/04 |
...
|------|------|---------|------|------|
| M-01-01/03 | 楽天検索 client-side Free回数制限実装 | M-6 | S | [ ] |
| M-01-02/03 | CI JWT検証チェックゲート追加 | M-7 | S | [ ] |
| M-01-03/03 | expo-updates 導入 & app.json設定 | MS7#1 | S | [ ] |

### M-01-01/03: 楽天検索 client-side Free回数制限

- **対象**: `genba-note/src/components/unitPrice/MaterialSearchModal.tsx`
- **旧Issue M-6 の対応状況**:
  - **server-side（実装済み）**: `supabase/functions/rakuten-search/index.ts` で `DAILY_LIMIT=100`（全ユーザー共通上限）+ `auth.uid` ベース日次制限を適用中。ただし Free/Pro 差分はなく共通上限のみ
  - **client-side（未実装）**: `FREE_RAKUTEN_SEARCH_DAILY_LIMIT = 5` は `freeTierLimitsService.ts` に定義済み、`canSearchRakuten()` 関数も実装済みだが、MaterialSearchModal の楽天検索フローで未使用
- **修正**: MaterialSearchModal 内の楽天検索実行前に、Freeユーザーは `canSearchRakuten()` で日次回数チェックを行い、超過時はPaywallへ誘導（AI検索の `canSearchAi()` と同じパターンを踏襲）
- **完了条件**: `rg "canSearchRakuten" genba-note/src/components/unitPrice/MaterialSearchModal.tsx` → 1件以上ヒット + 関連テスト追加・パス

### M-01-02/03: CI JWT検証チェックゲート

- **対象**: `.github/workflows/test.yml`
- **現状**: `supabase/config.toml` に `verify_jwt = true` は設定済みだが、CIでこの設定が維持されていることを検証するステップがない
- **修正**: test.yml に `verify_jwt = true` を grep でチェックするステップを追加
...
- **現状**: expo-updates が未インストール、app.json に updates 設定なし
- **修正**: `npx expo install expo-updates` + app.json に updates 設定追加
...
| Task | 内容 | 旧Issue | 規模 | 状態 |
|------|------|---------|------|------|
| M-02-01/07 | Apple Developer Program 登録（$99/年） | MS4 | M | [ ] |
| M-02-02/07 | Google Play Developer 登録（$25） | MS4 | M | [ ] |
| M-02-03/07 | EAS Submit 設定（eas.json 記入） | C-2 | S | [ ] |
...
- **現状**: `submit.production` が空オブジェクト `{}`
- **修正**: `appleId`, `ascAppId`, `appleTeamId`, `serviceAccountKeyPath` を記入
- **前提**: M-02-01/07, M-02-02/07 完了
- **完了条件**: `jq '.submit.production.ios | .appleId, .ascAppId, .appleTeamId | select(. == "")' genba-note/eas.json` → 出力なし
...
| Task | 内容 | 旧Issue | 規模 | 状態 |
|------|------|---------|------|------|
| M-03-01/04 | genba-note.app ドメイン取得 | RODOAPPU 4.2 | S | [ ] |
| M-03-02/04 | 静的ホスティング設定（GitHub Pages / Vercel 等） | RODOAPPU 4.2 | S | [ ] |
| M-03-03/04 | プライバシーポリシー & 利用規約の HTML 作成・デプロイ | RODOAPPU 5.1 | M | [ ] |
| M-03-04/04 | URL公開確認（HTTP 200 疎通テスト） | RODOAPPU 5.1 | S | [ ] |
...
| M-04-12/13 | 説明文・キーワード最終レビュー | M-13/MS5 | S | [ ] |
| M-04-13/13 | アプリ名・サブタイトル最終確認 | RELEASE_TODO | S | [ ] |
...
### M-04-12/13: 説明文・キーワード最終レビュー

- docs/store-metadata-ja.md のドラフトを最終レビュー
- **完了条件**: 両ストアの説明文・キーワードが入力済み

### M-04-13/13: アプリ名・サブタイトル最終確認

- App Store: アプリ名「GenBa Note」、サブタイトル「建設業の見積書・請求書をかんたん作成」（30文字以内）
- Google Play: アプリ名「GenBa Note - 建設業見積・請求書」（30文字以内）
- docs/store-metadata-ja.md のドラフトを最終レビュー
- **完了条件**: 両ストアにアプリ名・サブタイトルが入力済み
...
**M-01/07 コード修正:**
- [ ] M-01-01/03: `canSearchRakuten()` が MaterialSearchModal で使用されている
- [ ] M-01-02/03: CI で `verify_jwt = true` がチェックされている
- [ ] M-01-03/03: `expo-updates` がインストール済み + app.json に updates 設定あり
- [ ] `cd genba-note && npx jest` → 0 failures
- [ ] `cd genba-note && npx tsc --noEmit` → exit code 0
- [ ] `deno check supabase/functions/*/index.ts` → exit code 0

**M-02/07 外部サービス:**
- [ ] M-02-01/07: App Store Connect でアプリが存在 + Apple Team ID 取得済み
- [ ] M-02-02/07: Google Play Console でアプリが作成済み + サービスアカウント JSON 取得済み
- [ ] M-02-03/07: EAS Submit 設定（`eas.json`）の全フィールドが非空
...
- [ ] M-03-03/04: PP・利用規約 HTML がデプロイ済み + 必須事項が記載されている
...
- [ ] M-04-12/13: 説明文・キーワードが両ストアに入力済み
- [ ] M-04-13/13: アプリ名・サブタイトルが両ストアに入力済み
...
|-----------|---------|------|------|
| M-01/07 コード残修正 | 3 | `[Claude]` | 未着手 |
| M-02/07 外部サービス設定 | 7 | `[Human]` | 未着手 |
| M-03/07 ドメイン&法務 | 4 | `[Human/Both]` | 未着手 |
| M-04/07 ストア申請準備 | 13 | `[Human/Both]` | 未着手 |
