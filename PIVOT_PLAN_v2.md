...
| C2 | material research UI 全削除（`src/components/unitPrice/Material*`, `AiSearch*`, `AiPriceItemCard`, `materialSearchLimitUtils`, `aiSearchViewState` + 関連tests） | UI | — | **未着手** |
...
| C3-rest | **残 consumer 剥がし + plain `isPro` 残存除去**: 実測で C3-rest 完了条件 rg が検知する **計 18 source ファイル + 対応テスト**（C2 で削除予定の 6 UI ファイルは別扱い）。内訳: `app/document/preview.tsx`, `app/document/[id].tsx`（`useDocumentEdit(documentId, documentType, true)` の `true` 引数を削除、caller cleanup）, `src/hooks/index.ts`, `src/hooks/useDocumentEdit.ts`, `src/domain/csvExport/csvFileService.ts`, `src/domain/document/documentService.ts`, `src/domain/document/conversionService.ts`, `src/constants/templateOptions.ts`（`isProTemplate` 関数削除 + `resolveTemplateForUser` / `getSelectableTemplateOptions` の `isPro` 引数除去 + L86 `disabled: !isPro && option.requiresPro` 分岐削除 + `requiresPro` プロパティ/型定義削除）, `src/constants/errorMessages.ts`, `src/pdf/index.ts`, `src/pdf/types.ts`, `src/pdf/pdfGenerationService.ts`, `src/components/common/ActionSheetModal.tsx`, `src/components/document/TemplatePickerModal.tsx`, `src/components/settings/TemplateSelectionSection.tsx`（`getSelectableTemplateOptions(true)` caller cleanup）, `src/storage/index.ts`, `src/storage/secureStorageService.ts`, `src/types/index.ts`。対応テスト: `__tests__/constants/templateOptions.test.ts`（no-op 振る舞い検証テストの最終整理）, `__tests__/hooks/useDocumentEdit.test.ts` の materialResearch 連携削除。詳細は下記「C3 consumer 剥がし 対象詳細」参照 | domain consumer | **Codex review #1** | **未着手** |
| C4 | subscription ドメイン 全削除（`src/subscription/` ディレクトリ全体, `src/hooks/useProStatus.ts`, `src/hooks/useDailySearchUsage.ts`, `src/types/subscription.ts`）+ 当該ドメインに依存するテストの同時修正: `__tests__/subscription/`（残存分全削除）, `__tests__/hooks/useProStatus.test.ts`（M1-4 で stub 版に書き換え済 → 削除）, `useDailySearchUsage.test.ts`（削除）, `__tests__/storage/secureStorageService.test.ts`（`SubscriptionCache` / `SUBSCRIPTION_STORE_KEYS` / subscription cache API 参照テスト削除）, `__tests__/domain/document/documentService.test.ts`（`freeTierLimitsService` / `FREE_DOCUMENT_LIMIT` 依存テスト削除）, `__tests__/domain/document/conversionService.test.ts`（`freeTierLimitsService` 依存テスト削除）, `__tests__/domain/csvExport/csvFileService.test.ts`（`@/subscription/proAccessService` 依存テスト削除）, `__tests__/pdf/pdfGenerationService.test.ts`（`checkProStatus` 依存テスト削除）。C3-rest で consumer ゼロ化・C4 でテストも同時更新のため C4 完了時 green。 | domain + tests | — | **未着手** |
| C5 | materialResearch ドメイン 全削除（`src/domain/materialResearch/`, `src/hooks/useMaterialSearch.ts`, `useAiPriceSearch.ts`, `src/types/materialResearch.ts`）+ 当該ドメインに依存するテスト: `__tests__/domain/materialResearch/`（全削除）, `__tests__/hooks/useMaterialSearch.test.ts`（削除）, `useAiPriceSearch.test.ts`（削除）。**注: `src/domain/unitPrice/searchService.ts` は単価表の純粋検索・ソート機能（materialResearch 無関係）なので削除対象に含めない**。 | domain + tests | — | **未着手** |
| C6 | auth + supabase + preflight 削除（`src/domain/auth/`, `__tests__/domain/auth/`, `scripts/preflightEnv*`, `__tests__/scripts/preflightEnv.test.ts`, プロジェクトルート `supabase/`）+ `package.json` から `@revenuecat/purchases-react-native`, `@supabase/supabase-js`, `expo-auth-session`, `expo-web-browser` 削除 | storage / config | **Codex review #2** | **未着手** |
| C7 | 残 config 内部整理（C3-rest〜C6 で触れなかった dead re-export / dead import / dead type の残骸整理のみ） | config | — | **未着手** |
| C8 | utils / config 剥がし（`src/utils/environment.ts` の Supabase URL 検査削除, `src/utils/legalLinkHandlers.ts` の paywall 導線削除, `genba-note/.env.example` の Supabase / RevenueCat 行削除, `genba-note/eas.json` の `SUPABASE_URL` / `PUBLISHABLE_KEY` / `REVENUECAT_PUBLIC_KEY` 削除） | utils / config | — | **未着手** |
| C9 | `package-lock.json` / `deno.lock` 再生成（`cd genba-note && npm install`、`deno.lock` は Supabase 撤去に伴いクリーンアップ） | config | — | **未着手** |
| C10 | 残テスト最終調整: `__tests__/pdf/cleanupOrphanedPdfCache.test.ts`, `__tests__/utils/environment.test.ts`, `__tests__/app/settings-legal-links.test.ts` を Acceptance 通る状態に整える + M1 Acceptance 全5項目（test / lint / rg grep / tsc / 手動起動）確認 | tests | **Codex review #3** | **未着手** |

**C3 consumer 剥がし 対象詳細**（C3-pre / C3-rest を統合した完全版、実装時の抜け漏れ防止）:

凡例: `[✅ C3-pre]` = M1-1〜M1-8 で実装済 / `[⬜ C3-rest]` = 未着手

| ファイル | 剥がす内容 | 担当 |
|---|---|---|
| `app/(tabs)/index.tsx` | 上限表示・Pro切替UI削除 | [✅ C3-pre] M1-1, M1-5 |
| `app/(tabs)/settings.tsx` | Pro関連ボタン削除、legal links 保持 | [✅ C3-pre] M1-1, M1-7 |
| `app/(tabs)/prices.tsx` | Pro gate / 上限表示 削除済み、material research consumer 側は未了 | [部分完了] M1-1, M1-5 で Pro 側は完了。材料検索 `MaterialSearchModal` / research handlers は C2 削除 + C3-rest で剥がす |
...
| `app/document/[id].tsx` | `useProStatus` 削除 / `useDocumentEdit(..., true)` の isPro 引数 caller cleanup | [部分完了] M1-1, M1-8 で `useProStatus` 除去は完了。**`useDocumentEdit` が isPro 引数を取る以上、C3-rest で useDocumentEdit 引数削除と同時に caller 側 `true` リテラルも削除する必要あり** |
| `app/document/preview.tsx` | `useProStatus` 削除、`resolveTemplateForUser` の isPro 分岐削除 | [⬜ C3-rest] ※M1-1 / M1-3a で header と paywall redirect は削除済、stub `useProStatus()` 呼び出しと `isPro` 分岐が残存 |
| `app/customer/[id].tsx` | `useProStatus` 削除 | [✅ C3-pre] M1-1, M1-6 |
| `src/components/settings/TemplateSelectionSection.tsx` | (a) `getSelectableTemplateOptions(true)` の `true` 引数削除 / (b) L68-72 の `option.requiresPro && (...)` PRO バッジ branch 削除（`requiresPro` フィールド撤去に追従） | [部分完了] M1-7 で一部整理済だが、`true` リテラル caller と dead PRO バッジ UI branch が残存するため C3-rest で両方除去 |
| `src/components/document/edit/SaveActionSheet.tsx` | Pro 制限削除 | [✅ C3-pre] M1-8 |
| `src/components/document/TemplatePickerModal.tsx` | Pro テンプレ制限削除（`getSelectableTemplateOptions(isPro)` の isPro prop 依存除去 + L83 の `option.requiresPro && (...)` PRO バッジ branch 削除） | [⬜ C3-rest] M1-7 の templateOptions 側整理の影響で機能的には no-op だが、`isPro` prop 依存と dead PRO バッジ UI branch が残存するため C3-rest で完全除去 |
| `src/components/common/ActionSheetModal.tsx` | (a) `ActionSheetOption.isPro` prop 型定義削除 (L36) / (b) L118 近辺の `option.isPro` による Pro badge branch 削除 | [⬜ C3-rest] M1-1〜M1-8 で未変更、`isPro` prop 定義と dead Pro badge branch が残存するため C3-rest で除去 |
| `src/components/unitPrice/UnitPriceEditorModal.tsx` | 材料検索導線削除 | [⬜ C3-rest] ※C2 で material research UI を削除した後に剥がす |
| `src/components/document/edit/LineItemList.tsx` | 材料検索ボタン削除 | [⬜ C3-rest] ※C2 連動 |
| `src/components/unitPrice/index.ts` | Material* export 削除 | [⬜ C3-rest] ※C2 連動 |
| `src/domain/document/documentService.ts` | `isPro` 引数・`freeTierLimitsService` 参照削除 | [⬜ C3-rest] |
| `src/domain/document/conversionService.ts` | `freeTierLimitsService` 参照削除 | [⬜ C3-rest] |
| `src/domain/csvExport/csvFileService.ts` | `checkProStatus` 削除、Pro gating 削除 | [⬜ C3-rest] |
| `src/pdf/pdfGenerationService.ts` | `checkProStatus` 削除、watermark 常時 ON（Pro gating 削除） | [⬜ C3-rest] |
| `src/pdf/index.ts` | `checkProStatus` re-export 削除、`ProGateReason` / `ProGateResult` type export 削除 | [⬜ C3-rest] |
| `src/pdf/types.ts` | `@/subscription/types` re-export 削除、`ProGateReason` / `ProGateResult` 型定義削除 | [⬜ C3-rest] |
| `src/constants/templateOptions.ts` | (a) `isProTemplate` 関数ごと削除 / (b) `resolveTemplateForUser` の `isPro` 引数除去 / (c) `getSelectableTemplateOptions` の `isPro` 引数除去 + L86 の `disabled: !isPro && option.requiresPro` 分岐削除 / (d) `TEMPLATE_OPTIONS` 各エントリの `requiresPro: false` フィールドおよび型定義から `requiresPro` プロパティ削除（caller の PRO バッジ branch 削除に連動） | [部分完了] M1-7 で呼び出し側を `isPro=true` 等価へ誘導済み。**`isProTemplate` 関数、他2関数の `isPro` 引数、`requiresPro` フィールド・型定義が残存**しているため C3-rest で除去 |
| `src/constants/errorMessages.ts` | `@/subscription/types` import 削除、`PRO_GATE_MESSAGES` / `SUBSCRIPTION_ERROR_MESSAGES` / `getProGateMessage` / `getSubscriptionErrorMessage` 削除 | [⬜ C3-rest] |
| `src/hooks/index.ts` | `useProStatus` / `UseProStatusReturn` re-export 削除 | [⬜ C3-rest] |
| `src/hooks/useDocumentEdit.ts` | 材料検索連携削除 | [⬜ C3-rest] ※C2 連動 |
| `src/types/index.ts` | `subscription` / `materialResearch` re-export 削除 | [⬜ C3-rest] |
| `src/storage/secureStorageService.ts` | `@/types/subscription` import 削除、`SubscriptionCache` / `SUBSCRIPTION_STORE_KEYS` 参照と `getSubscriptionCache` / `saveSubscriptionCache` / `clearSubscriptionCache` 関数削除 | [⬜ C3-rest] |
| `src/storage/index.ts` | 上記 3 関数の re-export 削除 | [⬜ C3-rest] |
| `__tests__/hooks/useDocumentEdit.test.ts` | 材料検索連携テスト削除 | [⬜ C3-rest] ※C2 連動 |
| `__tests__/constants/templateOptions.test.ts` | `isProTemplate` / `resolveTemplateForUser` テスト削除（該当関数の `isPro` 引数除去と同時） | [部分完了] M1-7 で 102→37 行に縮小済。`isPro` 引数を使った no-op 検証テストが残存するため C3-rest で最終調整 |

**C3 完了条件（v11.4 拡張版）**: `cd genba-note` 後に以下を実行し、出力が空。plain `isPro`、subscription cache 関連 ID、再 export パス（`./subscription` / `./materialResearch`）、`useDocumentEdit(..., true)` caller、`getSelectableTemplateOptions(true)` caller、さらに `requiresPro`（テンプレ Pro 限定フィールド + PRO バッジ UI branch）まで網羅するよう `-e` パターンを拡張（false green を防ぐ）。
...
**現行 HEAD（v11.3 更新時点）のヒット内訳** (計 24 ファイル): C2 で削除予定の 6 UI (`MaterialSearchModal`, `MaterialSearchResultItem`, `AiSearchResultView`, `AiPriceItemCard`, `materialSearchLimitUtils`, `aiSearchViewState`) + C3-rest の 18 consumer (`app/document/preview.tsx`, `app/document/[id].tsx`, `src/components/common/ActionSheetModal.tsx`, `src/components/document/TemplatePickerModal.tsx`, `src/components/settings/TemplateSelectionSection.tsx`, `src/constants/errorMessages.ts`, `src/constants/templateOptions.ts`, `src/domain/csvExport/csvFileService.ts`, `src/domain/document/conversionService.ts`, `src/domain/document/documentService.ts`, `src/hooks/index.ts`, `src/hooks/useDocumentEdit.ts`, `src/pdf/index.ts`, `src/pdf/pdfGenerationService.ts`, `src/pdf/types.ts`, `src/storage/index.ts`, `src/storage/secureStorageService.ts`, `src/types/index.ts`)。C2 → C3-rest の順で 24 → 18 → 0 になる。

subscription / materialResearch ディレクトリ本体・hook 本体・types 本体は C4/C5 で削除するのでここでは残存してよい（consumer はゼロ参照）。rg は `-e` 複数指定で alternation を成立させる（`\|` は ripgrep ではリテラル `|` になるため使わない）。
